...
_global.SchemaRoller = function () {
    ...
    _createClass(Vector, [{
      ...
      value: function unshift() {
        ...
        items.forEach(function (item) {
          return _this12.setItemAt(0, item);
        });
        return this;
      }
    ...
    _createClass(Schema, [{
      ...
      value: function set(key, value) {
        ...
        var _setIfValid = function _setIfValid(_k, _v) {
          ...
          var _o = _object.get(_this14);
          _o[key] = _v;
          _object.set(_this14, _o);
          return _this14;
        };
        ...
        if (_exists(_childSigs[key])) {
          ...
          for (var _ in _pathKeys) {
            var k = _pathKeys[_];
            var _schema = void 0;
            var _key5 = this.path.length > 0 ? this.path + "." + k : k;
            ...
            if (_exists(_childSigs["*"])) {
              ...
              var _4 = _v.create(_schema = _childSigs, this.path + "." + key);
            }
            ...
            if (!_exists(_schema)) {
              ...
              if (!this.isExtensible) {
                return "element '" + _key5 + "' is not a valid element";
              }
              _schema = Schema.defaultSignature;
            }
            // handles child objects
            if ((typeof value === "undefined" ? "undefined" : _typeof2(value)) === "object") {
              value = _sH.setChildObject(_key5, value);
            }
            // handles absolute values (strings, numbers, booleans...)
            return _setIfValid(_key5, value);
          }
        }
  ...
  if (_schemaroller_.rx === null) {
    var _rx = "^((" + _sKeys.join("|") + ")+,?){" + _sKeys.length + "}$";
...
_global.wf.wfUtils.Fun.fromString = function (string) {
  var m = void 0;
  var _rx = /^function+\s\(([a-zA-Z0-9_\s,]*)\)+\s?\{+(.*)\}+$/;
  if ((m = string.replace(/\n/g, "").replace(/[\s]{2,}/g, "").match(_rx)) != null) {
    return _global.wf.wfUtils.Fun.factory([].concat(m[1], m[2]));
  } else {
    var _rx2 = "^Native::(" + Object.keys(this.natives).join("|") + ")+$";
    if ((m = string.match(new RegExp(_rx2))) != null) {
...
_global.wf.wfUtils.Obj.isOfType = function (value, kind) {
  var _fname = _global.wf.wfUtils.Fun.getFunctionName(kind);
